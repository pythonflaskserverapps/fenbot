...
class ChessboardPredictor(object):
  ...
  def getPrediction(self, tiles):
    ...
    # Prediction bounds
    a = np.array([x[0][x[1]] for x in zip(guess_prob, guessed)])
    ...
    pieceNames = ['1' if k == 0 else labelIndex2Name(k) for k in guessed] # exchange ' ' for '1' for FEN
    fen = '/'.join([''.join(pieceNames[i*8:(i+1)*8]) for i in reversed(list(range(8)))])
