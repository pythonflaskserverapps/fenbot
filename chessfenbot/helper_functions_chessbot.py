...
def predictSideFromFEN(fen):
  ...
  top_count_white = sum(list([ord(x) <= ord('Z') for x in top]))
  bottom_count_white = sum(list([ord(x) <= ord('Z') for x in bottom]))

  top_count_black = sum(list([ord(x) >= ord('a') for x in top]))
  bottom_count_black = sum(list([ord(x) >= ord('a') for x in bottom]))
